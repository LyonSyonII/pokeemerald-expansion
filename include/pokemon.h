...
struct Ability
{
    ...
    const u8 *description;
    // AI will predict the player's ability based to its aiRating. 
    // Without this flag the AI randomly assumes an ability with an even distribution between all possible abilities until one is confirmed. 
    // With this flag, it instead guesses proportionally to each ability's aiRating, making it far more likely to guess an ability like Water Absorb than Damp if both are options.
    s8 aiRating;
    // cannot be copied by Role Play or Doodle
    u8 cantBeCopied:1; 
    // cannot be swapped with Skill Swap or Wandering Spirit
    u8 cantBeSwapped:1; 
    // cannot be copied by Trace - same as cantBeCopied except for Wonder Guard
    u8 cantBeTraced:1; 
    // cannot be negated by Gastro Acid or Neutralizing Gas
    u8 cantBeSuppressed:1; 
    // cannot be overwritten by Entrainment, Worry Seed or Simple Beam (but can be by Mummy) - same as cantBeSuppressed except for Truant
    u8 cantBeOverwritten:1; 
    // can be bypassed by Mold Breaker and clones
    u8 breakable:1; 
    // doesn't work on an Imposter mon; when can we actually use this?
